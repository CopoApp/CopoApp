...
class Comment {
  static async list(postId) {
    try {
      ...
      for (let comment of comments) {
        const images = await db("comment_images")
    ...
    try {
      ...
      if (images?.length > 0) {
        for (const image of images) {
          const newImage = await db("comment_images").insert(
            {
  ...
  static async updateComment(commentInformation, addedImages) {
    ...
    try {
      ...
      const updatedCommentImages = await db
        .select("*")
        .from("comment_images")
  ...
  static async deleteImage(images) {
    ...
    for (let image of images) {
      ...
      await db("comment_images").where("id", image.id).del();
    }
  }
}
